...
function VtMap(mapdiv, infoboxdiv, sidebardiv, vname) {
	...
	var vesselinfoJson;
	var lastReload = 0;
	var specialIcon = new OpenLayers.Icon("http://images.vesseltracker.com/images/googlemaps/circle.png", new OpenLayers.Size(22, 22));
	var specialMarker;
	var currentMarker;
...
/**
	...
	this.init = function()
	{
		...
		map.addLayers( [ mapLayer, clusterLayer, iconLayer ]);
	}
	...
	this.setMoveEndEvent = function(yesOrNo)
	{
		moveEndEvent = yesOrNo;
	}
	...
	this.addVesselsCustomer = function(vesselinfo, lat, lon,zoom)
	{
		...
		sidebar_html = "";
		vessels = new Array(vesselinfoJson.length);
		...
		for (var i = 0; i < vesselinfoJson.length; i++)
		{
			...
			vessels[i] = addMouseOverMarker(i, lonlat, "http://images.vesseltracker.com/images/googlemaps/icon" + icon + ".png");
			var vtUrl;
			  if (imo) {vtUrl = "http://www.vesseltracker.com/"+language+"/Ships/"+ encodedName + "-" + imo + ".html";}
				 else { vtUrl = "http://www.vesseltracker.com/"+language+"/Ships/"+ encodedName +"-I" + shipId + ".html";}
			  ...
			  {
				  sidebar_html += "<a style='color:#555555' href='javascript:"+varname+".opener("+ i + ","+ 1 + ")'>" + encodedName + "</a><br/>";
			  } 
		}
...
//          		fillOpacity:"0.8",	//funktioniert nicht in IE
	...
	function doLoadXml()
	{
 	...
 	this.setInfoBoxShipLink = function(link){
		infoBoxShipLink = link;
	}
	...
	*/
	...
	this.opener = function (i, pan) {
		...
		if ( currentMarker != i)
		{
			...
			if(specialMarker)
			{
				specialMarker.lonlat = lonlat.transform(projwgs84,projmerc);
			}
			else 
			{
				specialMarker = new OpenLayers.Marker(lonlat.transform(projwgs84, projmerc),specialIcon);
			}
	 ...
	 function getLonLat (vessel){
		...
		function addMouseOverMarker (id, lonlat, iconUrl) {
			
			var lonlat = lonlat.transform(projwgs84, projmerc);
			var icon = new OpenLayers.Icon(iconUrl, new OpenLayers.Size(20, 20));
			var marker;
			
			if(divname_map == "gatshipMapSmall")
			{	
				var  pointFeature = new OpenLayers.Feature(iconLayer, lonlat,{icon:icon}); 
			    pointFeature.closeBox = true;
			    marker = pointFeature.createMarker();
			    if (!vesselinfoJson[id].lat)
			    {
			    	...
			    	description += vesselinfoJson[id].lastSeen + "</td></tr></table></html>";
				...
				marker.events.register("mouseout", pointFeature, point_mouseout);	    
			   	...
			   	//document.getElementById(divname_infobox).innerHTML = description;	
			  }
			}
			else
			{
...
function showInfoCustomer(i) {
		...
		if (imo)
		{
			vtUrl = "http://www.vesseltracker.com/"+language+"/Ships/"+encodedName+"-" + imo + ".html";
		}
		else
		{
			vtUrl = "http://www.vesseltracker.com/"+language+"/Ships/"+encodedName+"-I" + shipId + ".html";
		}
		...
		infoHtml += "<tr>";
		if (document.URL.indexOf("smd.de") == 0 || infoBoxShipLink == true)
		{
		...
		if (infoBoxShipLink == true)
		{
			infoHtml += "<td colspan='2' width='20%'><a href='"+vtUrl+"' target='_blank'><b>" + encodeShipname(vesselinfoJson[i].name) + "</b></td>";
		}
		else
		{

...
/**
	...
	this.changeBaseLayer= function(layername){
		...
		if (layername == "Google Physical") {
			mapLayer = new OpenLayers.Layer.Google("Google Physical", {
				type : google.maps.MapTypeId.TERRAIN,
				numZoomLevels : 19,
				animationEnabled:false
			});
		} else if (layername == "Google Streets") {
			mapLayer = new OpenLayers.Layer.Google("Google Streets", {
				numZoomLevels : 19,
				animationEnabled:false
			});
		} else if (layername == "Google Hybrid") {
			mapLayer = new OpenLayers.Layer.Google("Google Hybrid", {
				type : google.maps.MapTypeId.HYBRID,
				numZoomLevels : 19,
				animationEnabled:false
			});
		} else if (layername == "Google Satellite") {
			mapLayer = new OpenLayers.Layer.Google("Google Satellite", {
				type : google.maps.MapTypeId.SATELLITE,
				numZoomLevels : 19,
				animationEnabled:false
			});
		} else 	{	mapLayer = OSM_Layer}
	...
	this.init = function(showGoogle)
	{
		...
		OSM_Layer = new OpenLayers.Layer.TMS(
				"OpenStreetMap",
				urlArray,
				{
					numZoomLevels : 19,
	...
	this.setMyVesselInterval = function(time)
	{
		if($('#nextReload').find('input').attr('checked')!=undefined)
		{
			...
			$("#nextReload").html(g_nextReloadText +"<div onclick='javascript:"+varname+".setMyVesselInterval(60000);'>"+
		...
		else
		{
			...
			$("#nextReload").html("<div id='reloadNow' onclick='javascript:sendAjaxRequest(\"myVesselGroups\");sendAjaxRequest(\"myVessels\");"+varname+".displayLoadGif(\"block\");'>"+
					$("#translations input[name='reload_myvessels']").val()+"</div>"+
					"<div onclick='javascript:"+varname+".setMyVesselInterval(60000);'><input type='checkbox'/><span id='autoReload'>"+$("#translations input[name='reload_myvessels_auto']").val()+"</span></div>");	
		}
	}
	...
	/**
	 * Get the pixel from lonlat(WGS84)
	 */
	this.getPixelFromLonLat = function(lonlat){
		return map.getViewPortPxFromLonLat((lonlat).transform(projwgs84, projmerc));
	}
...
//Methode, die aus einer Json-Liste die passenden Schiffs-Marker in den IconLayer zeichnet	
	this.addVesselsCustomer = function(filter,sidebarPanTo)
	{
		...
		if ($("#mouseOverPopup_"+stillShowPopup).length >0)
		{
			$("#mouseOverPopup_"+stillShowPopup).remove();
		}
		$(".rectClass").remove();
		var followLonlat;
		...
		for (var a = 0; a < vessels.length; a++)
		{
			if(!vesselsToShow_filter || showVessel(vessels[a]))
			{
				...
				if(!myVesselMode && typeof vessels[a].independent  == "undefined")
				{
					...
					var sidebar_html="<a style='color:#555555' onfocus='blur' href='#'";
					if ($('#cockpit').length > 0)
					{
						var vesselPixel = map.getViewPortPxFromLonLat(me.getLonLat(vessels[a]).transform(projwgs84, projmerc));
						sidebar_html += " onclick='javascript:"+varname+".toggleDialog("+ vessels[a].shipId;		
						sidebar_html += ", "+ vesselPixel.x+", "+vesselPixel.y+");$(\"#mouseOverPopup_"+vessels[a].shipId+"\").remove();'";
						sidebar_html += " onmouseover = 'javascript:"+varname+".opener("+vessels[a].shipId+",false,true);"+varname+".paintMouseOverPopup("+vessels[a].shipId+",false)'";
						sidebar_html += " onmouseout = 'javascript:$(\"#mouseOverPopup_"+vessels[a].shipId+"\").remove();'";
						sidebar_html += " ondblclick='javascript:window.open(\""+vtUrl+"\")'>"+ me.encodeShipname(vessels[a].shipName) + "</a><br/>";	
					}
					else
					{
						sidebar_html += " onclick='javascript:"+varname+".opener("+ vessels[a].shipId+",false,true);return false;'";
						sidebar_html += " ondblclick='javascript:window.open(\""+vtUrl+"\")'>"+ me.encodeShipname(vessels[a].shipName) + "</a><br/>";	
					}
	...
	this.toggleDialog = function(shipId, x, y){
		if ($("#ship_"+shipId).length <1)
		{
			sendAjaxRequest("vesselDetails", shipId, x, y);
			me.displayLoadGif("block");
		}
		else
		{
			$("#ship_"+shipId).parent().parent().remove();
			$("#ship_"+shipId).remove();
		}
	}
		...
		for (var i = vesselsToShow_startIndex; i < vessels.length; i++)
		{
			...
			if (vessels[i].lonlat)
			{
				...
				if(vessels[i].shipId == currentMarkerShipId)
				{
					...
					specialMarker=null;
					specialIcon=null;
					me.opener(currentMarkerShipId,false,false);
				}
				...
				if (typeof nameGridArray[iii] != "undefined") {
					if (freePlace > (nameGridArray[iii][1].length / 3)) {
						var namePopup = new OpenLayers.Popup("popup_"+ nameGridArray[iii][2], nameGridArray[iii][0],
										new OpenLayers.Size(100, 30), "<div class='namePopup' ><span style='font-size:10px; position:absolute;' class='"+cssClassName+"'>"+nameGridArray[iii][1]+"</span></div>");
...
//	Create Clusterfeatures and give them a "type" attribute for their size and a label attribute for display their ship-count
		...
		for (i in mytrack)
		{
			if (typeof mytrack[i]["lon"] == "number") //notwendig, weil IE 8 auch die Methoden eines js-assoziativen Arrays  durchläuft 
			{
				...
				if(mytrack[i]["description"])
				{
			        ...
			        trackPointFeature.data.popupContentHTML = mytrack[i]["description"];
					...
					var trackPoint_mouseover = function (ev)
					{
						if (this.popup == null)
						{
							...
							this.popup.maxSize = new OpenLayers.Size(600,100);
							map.addPopup(this.popup, false);
							$('.olPopupContent').parents().css("border-radius","4px");
						}
						else
						{
							this.popup.toggle();
	...
	*/
	this.opener = function (shipId, pan, togglePopup) {
		if(clusterMode == true)
		{
			return;
		}
		...
		if(typeof g_vesselDetails != "undefined" && !g_vesselDetails)
		{
			specialMarker.events.register("click", specialMarker, function(e){
				...
				stillShowPopup = undefined;
				$("#mouseOverPopup_"+shipId).remove();
			});
		}
		else
		{
		...
		if(togglePopup)
		{
			specialMarker.events.register("mouseover", specialMarker, function(e){
				me.paintMouseOverPopup(vessel.shipId, false);
			});
	...
	this.getMiddleOfMap = function()
	{
		return map.getViewPortPxFromLonLat(map.getCenter());
	}
	
	this.paintMouseOverPopup = function(shipId, panToCenter)
	{
		stillShowPopup = shipId;
		var index = g_myVesselMode?0:1;
		var vessel = g_vessels[index]["_"+shipId];
		var vesselPixel = (panToCenter?me.getMiddleOfMap():map.getViewPortPxFromLonLat(me.getLonLat(vessel).transform(projwgs84, projmerc)));
		if (vessel == undefined)
		{
			vessel = g_vessels[2]["_"+shipId];
			index = 2;
		}
		var mouseOverPopup= "<div id='mouseOverPopup_"+vessel.shipId+"' class='mouseOverPopup' style='top:"+(vesselPixel.y-20)+"px;left:"+(vesselPixel.x+20)+"px;' >";
		...
		{
			mouseOverPopup+="<tr><td><nobr>Dest:</td><td>"+vessel.destination+"</nobr></td></tr>";
			mouseOverPopup+="<tr><td><nobr>ETA:</td><td>"+ (vessel.eta !=null ?vessel.eta : "-")+"</nobr></td></tr>";
		}
		if(vessel.lastSeenStr != null){
			mouseOverPopup+="<tr><td><nobr>Last seen:</td><td>"+ vessel.lastSeenStr +"</nobr></td></tr>";
		} else if(vessel.lastSeen != null){
			mouseOverPopup+="<tr><td><nobr>Last seen:</td><td>"+ vessel.lastSeen +"</nobr></td></tr>";
		}
		if (vessel.pic){
	...
	this.toggleMeasure = function(shipId, lon, lat)
	{
		...
		else
		{
			...
			$('#output_KM_'+currentMeasureShipId).html("0.00 ").next().html("KM");
			$('#output_NMI_'+currentMeasureShipId).html("0.00 ").next().html("NMI");
			$('#output_KM_'+currentMeasureShipId).parent().parent().parent().css("background", "url('http://images.vesseltracker.com/images/js/jquery/css/ui-lightness/images/ui-bg_gloss-wave_35_f6a828_500x100.png') repeat-x scroll 50% 50% #F6A828");
	...
	function createPopupText(type, mediaObject, vtUrl) {
		...
		if (type == "photo")
		{
		 ...
		 else if (type == "report")
		 {
			...
			if ( mediaObject['message'].length < 310 ){
				text += mediaObject['message'];
			} else {
				text += mediaObject['message'].substring(0,300) + '... &nbsp;';
				text += "<a target='_blank' href='" +vtUrl+ ".html'>more</a>"
			}
...
//			}
	...
	this.showDialogInfo = function(vessel) 
	{
		...
		var dialogHtml = "<div class='dialogInfo'><table width='100%' border='0' cellspacing='0' cellpadding='3'>";
		dialogHtml+="<tr>";
		dialogHtml+="<td width='30%'>";
		...
		{
			dialogHtml += varname+".opener("+vessel.shipId+",true,false);"+varname+".zoomTo(12)'>";
		}
		var status = (vessel.datasource?vessel.datasource:(vessel.status?vessel.status:"UNKNOWN"));
		dialogHtml+= "center View</a>";
		dialogHtml +="</td>"
		dialogHtml +="<td width='30%' class='showHideTrack' ><a href='javascript:showHideTracking("+ vessel.shipId+")'>"
		dialogHtml +=(g_mytracks[vessel.shipId]==undefined?"show":"hide")+" track</a>";
		dialogHtml +="</td><td width='40%'><div class='measureDist' onclick='javascript:"+varname+".toggleMeasure("+vessel.shipId+", "+vessel.lon +","+vessel.lat+")'>Distance Meter</div>";
		dialogHtml += '</td>';
		dialogHtml += '</tr>';
		dialogHtml += '</table>';
		dialogHtml += "<table border='0' cellspacing='0' height='95' cellpadding='3'>";		
		...
		dialogHtml +="<td><table class='measureOutput'><tr><td id='output_KM_"+vessel.shipId+"'></td><td class='measureUnit'></td></tr>";
		dialogHtml += "<tr><td id ='output_NMI_"+vessel.shipId+"'></td><td class='measureUnit'></td></tr></table></td></tr>";
...
//             	}
	...
	function handleMeasurements(event) {
		...
		if (units == "m") measure = measure/1000;
		var out = measure.toFixed(2);
		$('#output_KM_'+currentMeasureShipId).html(out);
		out = (measure * 0.540).toFixed(2);
		$('#output_NMI_'+currentMeasureShipId).html(out);
	}	
}
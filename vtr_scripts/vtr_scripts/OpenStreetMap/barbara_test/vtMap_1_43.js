...
//Methode, die aus einer Json-Liste die passenden Schiffs-Marker in den IconLayer zeichnet	
	this.addVesselsCustomer = function(filter,sidebarPanTo)
	{
		...
		if (vesselsToShow_showNames)	
		{
			...
			for (var p = 0; p < vesselsToShow_vessels.length;p++)
			{
				var rectWidth = 80;
				var rectHeight = 18;
				var Xoffset = 5;
				var Yoffset = -33;
				var vessel = vesselsToShow_vessels[p];
				if (vessel != undefined)
				{
					if (vessel.pixel == undefined)
					{
						console.debug("Pixel nicht berechnet für "+vessel.shipName);
					}
					else
					{
						vessel.rectangle = createRectangle(vessel.pixel, rectWidth,rectHeight, Xoffset, Yoffset);
//						drawRectangle(vessel.rectangle, rectWidth, rectHeight);
					}
					...
					if (!myVesselMode)
					{
						for (var d = 0;d < compare_rectangles.length; d++)
						{
							...
							{
								if (intersect(vessel.rectangle, compare_rectangles[d]))
								{
					...
					if(noIntersection)
					{
						...
						compare_rectangles.push(vessel.rectangle);
						var namePopup = new OpenLayers.Popup("popup_"+ p , vessel.lonlat, new OpenLayers.Size(100,30), "<div>"+ outlineText(me.encodeShipname(vessel.shipName, true))+"</div>");
...
//			}
	...
	{
		a.pixel = map.getViewPortPxFromLonLat(a.lonlat); 
		b.pixel = map.getViewPortPxFromLonLat(b.lonlat); 

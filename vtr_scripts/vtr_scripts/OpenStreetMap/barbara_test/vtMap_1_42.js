	...
	this.showIconsFrom = function(){
		...
		for (var i = vesselsToShow_startIndex; i < vessels.length; i++)
		{
			...
			if (lonlat && show)
			{
				...
				if(g_showVesselNames || (maxNumberByShowVesselName != undefined && vessels.length < maxNumberByShowVesselName) )
				{
						var namePopup = new OpenLayers.Popup("popup_"+i, lonlat, new OpenLayers.Size(2,5), "<div>"+ me.encodeShipname(vessels[i].shipName, true)+"</div>");

...
/**
	...
	this.init = function(showGoogle)
	{
		...
		if (showGoogle) {
			...
			// füge die Layers in der Reihenfolge von unten- nach obenliegend
			// der map hinzu

			map.addLayers( [ mapLayer, polygonLayer,
					trackingAndClusteringLayer, trackPointLayer, iconLayer,
					followIconLayer, gmap, gphy, ghyb, gsat ]);
	...
	this.setMyVesselMode = function(mvMode)
	{
		...
		myVesselMode = mvMode;
		me.setMoveEndEvent(mvMode!=true);
...
//Methode, die aus einer Json-Liste die passenden Schiffs-Marker in den IconLayer zeichnet	
	...
	this.showIconsFrom = function(){
		...
		for (var i = vesselsToShow_startIndex; i < vessels.length; i++)
		{
			...
			var lonlat = me.getLonLat(vessels[i]);
			var show = true;
			
			if(vesselsToShow_filter) // z.B. Filterdiv aus der Cockpit.html
			{
				show = showVessel(vessels[i]);
			}
							
			if (lonlat && show)
			{
				lonlat.transform(projwgs84,projmerc);
				...
				addMouseOverMarker(vessels[i].shipId, lonlat, "http://images.vesseltracker.com/images/googlemaps/icon" + vessels[i].icon + ".png", new OpenLayers.Size(20, 20), vessels[i].shipName);
				
				if (map.getZoom() >= POLYGON_MINZOOM ||myVesselMode)
				{
					polygonLayer.addFeatures([me.addPolygon(vessels[i])]);
				}
				if(g_showVesselNames )
				{
						var namePopup = new OpenLayers.Popup("popup_"+i, lonlat, new OpenLayers.Size(2,5), "<div><span style='font-weight:bold'>"+ me.encodeShipname(vessels[i].shipName, true)+"</span></div>");
						map.addPopup(namePopup, false);
						namePopup = null;
				}
				if(!myVesselMode && typeof vessels[i].independent  == "undefined")
				{
					var vtUrl =  "http://" + window.location.host + "/"+ g_language+"/Ships/"+me.encodeShipnameForUrl(vessels[i].shipName);
					vtUrl += (vessels[i].imo?"-" + vessels[i].imo + ".html":"-I" + vessels[i].shipId + ".html");
					var sidebar_html="";
					sidebar_html += "<a style='color:#555555' href='javascript:"+varname+".opener("+ vessels[i].shipId;
					sidebar_html += (vesselsToShow_sidebarPanTo?",true)'":")'");
					sidebar_html += " ondblclick='javascript:window.open(\""+vtUrl+"\")'>"+ me.encodeShipname(vessels[i].shipName) + "</a><br/>";	
				...
				if(document.getElementById('divContext')!=null) // context-menu only exists in CockpitMap
				{
					...
					if(filter && g_highlight_myVessels == true)
					{
						...
						{
							addMouseOverMarker(vessels[i].shipId, lonlat, "http://images.vesseltracker.com/images/circleblue.png", new OpenLayers.Size(27, 27));
						}
					}
					...
					{
						if(map.calculateBounds().contains(lonlat.lon,lonlat.lat))
						{
							followLonlat = lonlat;
						}
						addMouseOverMarker(vessels[i].shipId, lonlat, "http://images.vesseltracker.com/images/googlemaps/circle_red.png", new OpenLayers.Size(30,30));
					}
	...
	*/
	 ...
	 this.reloadData = function(restClock)
	{
		 ...
		 else
		 {
			  ...
			  * 7:g_shipIds of additional ships (satellite, lastposition, searchResults)
			  * 8:g_pageSource 
			  * 9:g_lastSeenTimeSpan: for super Account the timeLimit how far into the past shippositions are requested
			  * */
			 sendAjaxRequest("allVessels", lastReloadBounds.top, lastReloadBounds.left, lastReloadBounds.bottom, lastReloadBounds.right, map.getZoom(), params['type'],shipIds, pageSource, lastSeenTimeSpan);
			 firstLoad=false;
		 }
	...
	this.opener = function (shipId, pan) {
		...
		if(typeof g_vesselDetails != "undefined" && !g_vesselDetails)
		{
			...
			specialMarker.events.register("mouseover", specialMarker, function(e){
				var contentDiv = "<div id='mouseOverSpecialMarker' ><nobr>";
				if (vessel.flagid){
				...
				contentDiv+="</div>";
				$(this.icon.imageDiv).append(contentDiv);
...
//			var newReloadBounds = map.getExtent().transform(projmerc, projwgs84);
//			var contained = (lastReloadBounds != null?lastReloadBounds.containsBounds(newReloadBounds,false,true):false);
//			
//			if (shouldReload && contained && map.getZoom() != POLYGON_MINZOOM )
//			{
//				shouldReload = false;
//			}
//		}		
...
this.showDialogInfo = function(vessel) 
	{
		...
		if (g_vessels[0]["_"+vessel.shipId] == undefined && g_shipIds.indexOf(vessel.shipId)< 0)
		{
